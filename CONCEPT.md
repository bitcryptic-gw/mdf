...
**Rail is inferred, not declared.** The reference server does not emit `payment.rail`; a consumer maps `chain` to a rail itself, with `base` and `ethereum` implying x402 and `lightning` implying L402. This works today and breaks the moment two rails share a settlement network, which MPP settling on an EVM chain would immediately cause. `rail` is therefore defined as an optional field: servers emitting it remove an inference from every consumer, and servers omitting it remain conformant.
...
**Two fields are recommended but not yet implemented.** `resource`, the absolute URL being priced, is the one piece of a 402 a consumer cannot reconstruct once the response is held out of band — queued, retried, logged, or passed between processes. `payment.expires_at` states how long the offer stands; without it, any consumer that defers a payment is guessing. Both are defined in the schema and marked as not currently emitted by the reference server.
